...
print(new_path)
os.chdir(new_path)
# when run though script
#before-pipeline-data-input/cleaned-data/calcuate_other_d_rlk_test.py
#import Osa_pfam
Osa_nsg_pfam = pd.read_csv('Osa_nsg_pfam_parser.csv')
Osa_psg_pfam = pd.read_csv('Osa_psg_pfam_parser.csv')
print(len(Osa_nsg_pfam))
print(len(Osa_psg_pfam))
Osa_pfam = pd.concat([Osa_nsg_pfam, Osa_psg_pfam]).reset_index(drop=True)
print(len(Osa_pfam))

#extract unique domain list in pfam
Osa_domain = Osa_pfam['hmm_name'].unique().astype(str)
Osa_domain_list=[]

#need a for loop here
for index in range(Osa_domain.shape[0]):
   print('Column Number : ', index)
   # Select column by index position using iloc[]
   domain = Osa_domain[index]
   Osa_domain_list.append(domain)
#import domain list from github weebsite
